...
local function detect_iface()
  ...
  if iface_exists("eth0") then return "eth0" end
  if iface_exists("wlan0") then return "wlan0" end
  return nil
end
...
local function insert_devices(db, ts, count)
  local stmt = db:prepare(
    "INSERT INTO devices (ts, device_count) VALUES (?, ?);"
  )
...
io.stdout:write(string.format(
  "net_devices.lua: iface=%s count=%d\n",
  INTERFACE, count
))
